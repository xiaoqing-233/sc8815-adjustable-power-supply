...
#define SC8815_PSTOP_PIN    SC_PSTOP_Pin 
#define SC8815_PSTOP_PORT   SC_PSTOP_GPIO_Port

#define SC8815_CE_PIN       SC_CE_Pin
#define SC8815_CE_PORT      SC_CE_GPIO_Port
...
#define SCHW_FB_RDOWM           5600       //RDOWM 为从 FB 连接到 GND 之间的电阻值
#define SCHW_VBAT_RUP           270000      // 上拉电阻 210k
#define SCHW_VBAT_RDOWM         15000       // 下拉电阻 15k

// VBUS 输出端 (120k + 6.8k) / 6.8k ≈ 18.647
// 芯片以为是 12.5x，实际是 18.647x
#define MY_REAL_VBUS_RATIO   ((float)(SCHW_FB_RUP + SCHW_FB_RDOWM) / SCHW_FB_RDOWM)

//// VBAT 输入端 (210k + 13k) / 13k ≈ 17.154
//// 芯片以为是 12.5x，实际是 17.154x
//#define MY_REAL_VBAT_RATIO   ((float)(240000 + 15000) / 15000.0f)
//	
//#define MY_REAL_VBUS_RATIO   12.1f

// 同理，电池端通常也是 12.5
#define MY_REAL_VBAT_RATIO   ((float)(SCHW_VBAT_RUP + SCHW_VBAT_RDOWM) / SCHW_VBAT_RDOWM)
// 芯片寄存器里我们设置的档位 (必须在 HardwareInit 里设为 12.5x)
#define CHIP_PRESET_RATIO    12.5f
...
uint8_t SC8815_Init_Check(void); //自检（没有调用）
void SC8815_Start_OTG_Mode(void); //主程序调用的初始化
void SC8815_EnableOutput(uint8_t enable); //使能输出
#ifdef __cplusplus
}
#endif // __cplusplus
#endif // !SC8815_H
